# RescueLink — Phase 4: Nearby SOS / Rescue status / Maps
...
ปุ่มเปิดตำแหน่งส่งพิกัดไปยังแอปแผนที่บน Android แสดงอายุ/ความแม่นยำของพิกัดและข้อความกรณีเปิดไม่ได้ การดูแผนที่โดยไม่มีอินเทอร์เน็ตต้องดาวน์โหลดข้อมูลไว้ในแอปแผนที่ก่อน ยังไม่มีแผนที่ออฟไลน์ใน RescueLink

ติดตั้ง Phase 4 รุ่นแก้ไขเดียวกันทั้งสองเครื่อง แล้วกด “เริ่มรับส่ง SOS / เชื่อมต่ออัตโนมัติ” ทั้งสองฝั่ง แอปค้นหาและเชื่อมต่อ peer ให้ก่อนรับสถานะ (ยังไม่ใช่ SOS broadcast แบบไม่เชื่อมต่อ) รุ่นแก้ไขใช้ Android connected-device foreground service พร้อม notification และ partial wake lock ระหว่าง session เพื่อคง Nearby ขณะสลับแอปหรือดับหน้าจอ กดหยุดเมื่อเลิกใช้งานเพื่อลดการใช้แบตเตอรี่ การปัดแอปทิ้ง/force-stop/OS kill ยังไม่รองรับการเริ่ม session ใหม่เอง คู่มือและผลทดสอบอยู่ใน [Phase 4](docs/PHASE_4_TEST.md)

สถานะ SOS และตัวนับอยู่ด้านบน พร้อมข้อความแจ้งในแอปและ Android notification เมื่ออนุญาตสิทธิ์แจ้งเตือน แจ้งครั้งแรกและเมื่อ revision เปลี่ยน ไม่แจ้งซ้ำทุก heartbeat ปุ่มคัดลอกพิกัดใช้ได้แม้ไม่มีข้อมูลแผนที่ออฟไลน์

ข้อความ SOS แบบเลือกผู้รับและ ACK จาก Phase 3 ยังใช้ได้เป็นช่องทางเพิ่มเติม ส่วนสถานะประกาศใช้ heartbeat ล่าสุด ไม่แสดงว่าเจ้าหน้าที่ตอบรับแล้ว

## ประวัติ Phase 3

ผลทดสอบ 2026-09-25: ผู้ใช้ยืนยันว่าการทดสอบบนโทรศัพท์จริงตามคู่มือ 5 หัวข้อผ่านทั้งหมด ได้แก่ เชื่อมต่อออฟไลน์, SOS, GPS/แชต, เก็บข้อมูลและยกเลิกเมื่อกลับมาเชื่อมต่อ, และกรณีใช้ GPS ไม่ได้ ดูรายละเอียดใน [บันทึก Phase 3](docs/PHASE_3_TEST.md)

Phase 3 เพิ่มหน้า **SOS / หน่วยช่วยเหลือ**: เลือกประเภทเหตุ ชื่อ จำนวนคน รายละเอียด และผู้รับที่เคยเชื่อมต่อ อ่าน GPS เมื่อกดปุ่มเท่านั้น (ส่งโดยไม่แนบพิกัดได้) ตรวจข้อมูลก่อนส่ง อัปเดตหรือยกเลิก SOS และเปิดแชตจาก Rescue Mode ได้ ข้อมูลและคิวส่งอยู่ใน SQLite พร้อม retry / ACK เดิม

ติดตั้งรุ่น Phase 3 ทั้งสองเครื่อง แล้วเชื่อมต่อจากหน้าหลักก่อนเลือกผู้รับ SOS หากออฟไลน์จะรอส่งเมื่อเชื่อมต่ออีกครั้ง **เครื่องปลายทางบันทึกแล้ว ไม่ได้หมายถึงเจ้าหน้าที่รับงานแล้ว** แอปยังรับส่งเฉพาะ foreground และยังไม่มี relay หรือการแจ้งเตือน background อ่าน [คู่มือทดสอบ Phase 3](docs/PHASE_3_TEST.md)

รายละเอียด Phase 1–2 ด้านล่างเป็นประวัติการพัฒนา
